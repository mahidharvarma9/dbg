...
void generateDBGMapping(const graph<vertex>& GA, bool isSym, bool useOutdeg, pvector<uintE>& new_ids, bool isPageRank, bool isDenseWrite) {
    ...
    int temp_k = 0;
    uint32_t start_k[num_threads][num_buckets];
    uint32_t bucket_sum[num_threads][num_buckets];
    uint32_t bucket_size[num_buckets]={0};
    uint32_t bucket_avg[num_buckets]={0};
    ...
    for ( int t = 0 ; t < num_threads; t++ ) {
        for ( int32_t j = num_buckets-1 ; j >= 0 ; j-- ) {
            ...
            const size_t& size = current_bucket.size();
            for ( uint32_t i = 0 ; i < size ; i++ ) {
                bucket_sum[t][j] += origG[current_bucket[i]].getOutDegree()+origG[current_bucket[i]].getInDegree();
                new_ids[ current_bucket[i] ] = k++;
            }
        }
    }
    ...
    for ( unsigned int j = 0 ; j < num_buckets ; j++ ) {
        for ( uint64_t i = 0 ; i < num_threads ; i++ ) {
            bucket_size[j]+= local_buckets[i][j].size();
            bucket_avg[j]+= bucket_sum[i][j];

            local_buckets[i][j].clear();
            //bucket_sum[i][j].clear();
        }
        avg_total+= bucket_avg[j];
        std::cout <<"Sum of outdegree of buckrt: "<<j<<"   "<<bucket_avg[j]<<std::endl;
        if(bucket_size[j]!=0) bucket_avg[j]/= bucket_size[j];
    ...
    avg_total/=num_vertices;

    cout<<endl<<endl;
    // std::cout << "Printing new_ids....." << std::endl;
    // for ( uint64_t i = 0 ; i < num_vertices ; i++ ) {
    ...
    std::cout << "Printing Bucket Averages....." << std::endl;
    for ( uint64_t i = 0 ; i < num_buckets ; i++ ) {
        std::cout <<"Average and Size of bucket No."<<i<<"     "<<bucket_avg[i]<<"   "<<bucket_size[i] << std::endl;
    }

    std::cout << "Total Average  " <<avg_total<< std::endl;
    cout<<endl<<endl;
